...
- Holybro Pixhawk Jetson Baseboard 在板内并行连接 Jetson 与 V6X：内部 Ethernet、
  `UART1 <-> TELEM2` 和 CAN。当前实机已验证 Ethernet/UDP MAVLink 2，V6X
  `192.168.0.3:14550` 广播到 `192.168.0.255:14550`；`/dev/ttyTHS1:921600`
  作为后备链路。这里没有外接 Jetson-飞控数据线，且该链路与 GR01 接收机无关。
...
Jetson Orin NX/Nano ─────┘
       │
       ├── board-internal Ethernet/UDP, read-only ─── Pixhawk V6X (primary)
       └── board-internal /dev/ttyTHS1 ────────────── Pixhawk V6X TELEM2 (backup)
...
3. **飞控平面**：G20/GR01 与 Pixhawk 传输标准飞控 MAVLink；Jetson 初期只读
   载板内部 Ethernet/UDP，THS1 作为尚未启用的后备。任何飞控写入能力都单独评审和测试。

优先使用直连视觉元数据平面的原因：不能把飞控“必然转发任意未知自定义消息”作为
设计前提。标准
...
如果台架证明 GR01 的空中端网口不能发送上行 IP 数据，再按届时确定的 PX4 固件版本
单独设计和验证飞控路由备选；不能把旧的 ArduPilot `TELEM1 -> TELEM2` 路由假设直接
套到当前 V6X。该备选必须先在 SITL 验证，再在拆桨、无载荷台架抓包测双向路由、
丢包率和延迟。不要直接使用飞控不认识的自定义消息 ID。
...
远程框选会同时启动一个 OpenCV CSRT/KCF/MIL 影子单目标跟踪器。若框选当帧已关联到
YOLO 火情轨迹，影子跟踪器使用更紧的正式检测框初始化，而不是较宽的操作员选择框；YOLO
轨迹健康时只返回正式 Track ID，短时漏检时立即切到影子手动框，并把该框作为检测器重捕获
提示，检测恢复后再切回正式 Track ID。影子跟踪器在 YOLO 正常期间若失效，会用最新正式
检测框重新初始化；后端不可用时则对本次选择停止反复重试并上报错误。手动跟踪失效和重捕获均有
时限，超时返回 `LOST/REJECTED`。手动框只影响跟踪显示与重捕获区域，不能生成授权挑战、
改变安全规则结论或触发载荷接口。
...
2026-07-14 已在独立 `QGroundControl-MultiDetect` 源码树的 `custom/` 中加入首个原生
自定义构建骨架：`FlyViewVideo` 在 QGC 原始视频本地叠加归一化目标框，只有操作员明确
进入“框选目标”状态时才接管触摸事件，从而避免与原有云台/相机跟踪手势冲突；右侧状态
面板显示任务阶段、目标、跟踪质量、19 项安全规则汇总、建议窗口、授权、载荷库存和
Jetson 健康；独立弹窗承载软件 HIL 授权。内置动态巡检仿真可演示候选火情、多帧确认、
跟踪、安全检查和短时授权，无需连接真机。

该里程碑现已完成 Windows Release 编译、桌面实跑，以及 Android ARM64 APK 构建和静态
验收。G20 台架包位于相邻源码树
`QGroundControl-MultiDetect/build-multidetect-android/MultiDetectGCS-G20-arm64-v8a-0.1.0-hil-debug-signed.apk`，
实际包名为 `com.multidetect.gcs`，只含 `arm64-v8a`，`aapt`、APK v3 签名、`zipalign`、
23 项 Android 单元测试及零错误 release lint 均已通过；证据见
`artifacts/evaluation/custom-qgc-android-g20-20260714T043822Z.json`。当前没有 ADB 设备，因此
尚未在 G20 安装或验证 1920x1200 渲染、H.265 硬解、RTSP 延迟和触摸坐标。其只读属性固定
`flightControlWritesEnabled=false`、`physicalReleaseEnabled=false`，编译定义也固定为
0；批准仿真挑战只进入 `DEPLOYMENT_READY / RELEASE INHIBITED`，没有串口、CAN、GPIO、
PWM、MAVLink 飞控控制或物理载荷路径。

2026-07-14 的隔离桌面 HIL 同时运行了自定义 QGC、真实 PX4 固定翼 SITL 和仅声明
Jetson `1/191` 的元数据驱动。QGC 等到 PX4 首次连接完成后，完成框选、状态、挑战绑定授权
和 ACK 并以 0 退出；Jetson 没有伪造 `1/1` 飞控心跳。只读路由允许参数/任务读取、只读
MAVLink FTP 和预检状态诊断，阻断 `SYSTEM_TIME`、未知命令及所有控制/写入，并保证
operator `TUNNEL` 不进入 PX4。完整证据见项目 README。下一步是把已验收 APK 安装到 G20，
完成分辨率实机渲染、签名链路与 RTSP/触摸台架，不直接进入载荷硬件。

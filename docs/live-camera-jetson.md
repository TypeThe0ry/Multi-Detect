...
默认要求至少1000帧和1800秒，且必须识别到 Orin NX/Nano、TensorRT/CUDA provider 与有效温度；
...
`pixhawk-check`、本机摄像头 Live 门禁测试和断链反向检查：
...
运行前后的所有者。它不替换或删除已有同名容器，只停止自己成功创建的容器。

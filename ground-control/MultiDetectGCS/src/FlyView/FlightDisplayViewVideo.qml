...
Item {
    ...
    Rectangle {
        ...
        PinchHandler {
            ...
            // MAV_CMD_SET_CAMERA_ZOOM with ZOOM_TYPE_STEP accepts only direction:
            // -1 for wide and +1 for tele. Accumulate the incremental scale changes
            // so small touch jitter does not flood the camera with zoom commands.
            property real zoomAccumulator: 1.0
            readonly property real zoomStepRatio: 1.1
            ...
            onScaleChanged: (delta) => {
                if (!_hasZoom || !active || delta <= 0) {
                    return
                }

                zoomAccumulator *= delta
                if (zoomAccumulator >= zoomStepRatio) {
                    _camera.stepZoom(1)
                    zoomAccumulator = 1.0
                } else if (zoomAccumulator <= (1.0 / zoomStepRatio)) {
                    _camera.stepZoom(-1)
                    zoomAccumulator = 1.0
                }
            }

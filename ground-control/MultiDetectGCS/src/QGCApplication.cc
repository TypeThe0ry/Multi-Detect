...
QGCApplication::QGCApplication(int& argc, char* argv[], const QGCCommandLineParser::CommandLineParseResult& cli)
    : QGuiApplication(argc, argv),
      ...
      _offlineMode(cli.offlineMode),
      _isolatedHilMode(cli.isolatedHilMode),
      ...
      _systemId(cli.systemId.value_or(0))
{
    ...
    if (_isolatedHilMode) {
        applicationName = QStringLiteral("%1 Isolated HIL").arg(QGC_APP_NAME);
    } else if (_offlineMode) {
        applicationName = QStringLiteral("%1 Offline").arg(QGC_APP_NAME);
    } else if (_runningUnitTests || _simpleBootTest) {
    ...
    if (_offlineMode || _isolatedHilMode) {
        const QString profileName =
            _offlineMode ? QStringLiteral("offline-settings") : QStringLiteral("isolated-hil-settings");
        isolatedSettingsPath = QDir(QStandardPaths::writableLocation(QStandardPaths::TempLocation))
                                   .filePath(QStringLiteral("%1/%2").arg(QGC_APP_NAME, profileName));
        if (!QDir().mkpath(isolatedSettingsPath)) {
            qCWarning(QGCApplicationLog) << "Could not create isolated settings path" << isolatedSettingsPath;
        }
    ...
    if (_runningUnitTests || _simpleBootTest || _offlineMode || _isolatedHilMode) {
    ...
    if (_offlineMode || _isolatedHilMode) {
        // A clean settings namespace alone is not sufficient because QGC defaults
        // to UDP and serial auto-connect. Force every connection trigger off before
        // SettingsManager constructs its Facts, and leave no saved link to start.
        settings.beginGroup(QStringLiteral("AutoConnect"));
        settings.setValue(QStringLiteral("autoConnectUDP"), false);
        settings.setValue(QStringLiteral("autoConnectPixhawk"), false);
        settings.setValue(QStringLiteral("autoConnectSiKRadio"), false);
        settings.setValue(QStringLiteral("autoConnectRTKGPS"), false);
        settings.setValue(QStringLiteral("autoConnectLibrePilot"), false);
        settings.setValue(QStringLiteral("autoConnectNmeaPort"), QStringLiteral("Disabled"));
        settings.endGroup();
        settings.setValue(QStringLiteral("LinkConfigurations/count"), 0);
        settings.setValue(QStringLiteral("forwardMavlink"), false);
        settings.setValue(QStringLiteral("sendGCSHeartbeat"), false);
        settings.setValue(QStringLiteral("telemetrySave"), false);
        settings.setValue(QStringLiteral("telemetrySaveNotArmed"), false);
        settings.setValue(QStringLiteral("apmStartMavlinkStreams"), false);
        settings.setValue(QStringLiteral("disableAllPersistence"), true);
        settings.setValue(QStringLiteral("firstRunPromptIdsShown"),
                          QString::number(QGCCorePlugin::kInitialSetupPromptId));
        settings.setValue(QStringLiteral("savePath"), QDir(isolatedSettingsPath).filePath(QStringLiteral("data")));
        settings.beginGroup(QStringLiteral("Video"));
        settings.setValue(QStringLiteral("streamEnabled"), false);
        settings.setValue(QStringLiteral("videoSource"), QString());
        settings.endGroup();
        settings.sync();
    }
...
void QGCApplication::_initForNormalAppBoot()
{
    ...
    if (_offlineMode) {
        linkManager->setConnectionsSuspended(tr("offline UI validation mode"));
    }
    linkManager->init();

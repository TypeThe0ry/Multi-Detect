...
void FactValueGrid::_loadValueData(QSettings& settings, InstrumentValueData* value)
{
    ...
    const QString factGroupName = settings.value(_factGroupNameKey).toString();

    // Existing telemetry-bar settings predate the display-only airspeed Fact.
    // Migrate only the stock compact AirSpd cell; user-selected raw airspeed
    // values elsewhere continue to expose the unmodified vehicle telemetry.
    if ((_settingsGroup == QStringLiteral("TelemetryBarUserSettings")) &&
        (factGroupName == InstrumentValueData::vehicleFactGroupName) &&
        (factName.compare(QStringLiteral("AirSpeed"), Qt::CaseInsensitive) == 0) &&
        (settings.value(_textKey).toString() == QStringLiteral("AirSpd"))) {
        factName = QStringLiteral("AirSpeedDisplay");
    }

    if (!factName.isEmpty()) {
        value->setFact(factGroupName, factName);
    }

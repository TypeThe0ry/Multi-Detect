...
namespace {
constexpr double kAirSpeedDisplayEnterThresholdMps = 2.0;
constexpr double kAirSpeedDisplayExitThresholdMps = 1.5;
}
...
void VehicleFactGroup::_handleVfrHud(const mavlink_message_t &message)
{
    ...
    const double airSpeedMps = qIsNaN(vfrHud.airspeed) ? 0.0 : static_cast<double>(vfrHud.airspeed);
    airSpeed()->setRawValue(airSpeedMps);

    // MS4525DO resolution is coarse around zero differential pressure. Keep the
    // raw fact untouched, but suppress its sub-resolution sign chatter in the
    // compact flight instrument. A larger negative value remains visible so a
    // reversed pitot/static connection is still obvious.
    if (_airSpeedDisplayActive) {
        _airSpeedDisplayActive = std::abs(airSpeedMps) > kAirSpeedDisplayExitThresholdMps;
    } else {
        _airSpeedDisplayActive = std::abs(airSpeedMps) >= kAirSpeedDisplayEnterThresholdMps;
    }
    airSpeedDisplay()->setRawValue(_airSpeedDisplayActive ? airSpeedMps : 0.0);

...
constexpr int kRtspTcpTimeoutMinUs = 1000000;
constexpr int kRtspTcpTimeoutMaxUs = 120000000;
...
GstElement* buildRtspSource(const QString& uri, const QUrl& sourceUrl, const Config& config, guint latencyMs)
{
    ...
    // The Multi-Detect camera/Jetson network uses TCP-interleaved RTSP. Some
    // embedded cameras accept UDP SETUP but never deliver RTP through the host
    // firewall, preventing GStreamer from reaching its TCP fallback. Keep the
    // upstream UDP+TCP behavior unless the custom app explicitly forces TCP.
    const bool forceTcp = qEnvironmentVariableIntValue("QGC_RTSP_FORCE_TCP") == 1;
    const GstRTSPLowerTrans rtspProtocols =
        forceTcp ? GST_RTSP_LOWER_TRANS_TCP
                 : static_cast<GstRTSPLowerTrans>(GST_RTSP_LOWER_TRANS_UDP | GST_RTSP_LOWER_TRANS_TCP);
    bool timeoutOk = false;
    const int configuredTcpTimeoutUs =
        qEnvironmentVariableIntValue("QGC_RTSP_TCP_TIMEOUT_US", &timeoutOk);
    const guint64 tcpTimeoutUs =
        timeoutOk && configuredTcpTimeoutUs >= kRtspTcpTimeoutMinUs &&
                configuredTcpTimeoutUs <= kRtspTcpTimeoutMaxUs
            ? static_cast<guint64>(configuredTcpTimeoutUs)
            : kRtspTcpTimeoutUs;
    ...
    g_object_set(source, "location", cleanLocation.constData(), "latency", latencyMs, "do-rtcp", TRUE,
                 "do-retransmission", config.doRetransmission ? TRUE : FALSE, "tcp-timeout", tcpTimeoutUs,

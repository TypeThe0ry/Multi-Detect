...
Item {
    ...
    property bool   _addAreaRouteOnDrag: false
    ...
    // Create a standard QGC Survey item from a screen-space rectangle. The
    // Survey item owns the GPS polygon and continuously rebuilds its native
    // fixed-wing coverage transects as the operator edits it afterward.
    function createAreaRoute(startPoint, endPoint) {
        var left = Math.min(startPoint.x, endPoint.x)
        var right = Math.max(startPoint.x, endPoint.x)
        var top = Math.min(startPoint.y, endPoint.y)
        var bottom = Math.max(startPoint.y, endPoint.y)

        if ((right - left) < ScreenTools.defaultFontPixelWidth
                || (bottom - top) < ScreenTools.defaultFontPixelWidth) {
            return
        }

        var topLeft = editorMap.toCoordinate(Qt.point(left, top), false /* clipToViewPort */)
        var topRight = editorMap.toCoordinate(Qt.point(right, top), false /* clipToViewPort */)
        var bottomRight = editorMap.toCoordinate(Qt.point(right, bottom), false /* clipToViewPort */)
        var bottomLeft = editorMap.toCoordinate(Qt.point(left, bottom), false /* clipToViewPort */)
        var center = editorMap.toCoordinate(Qt.point((left + right) / 2, (top + bottom) / 2), false /* clipToViewPort */)

        if (!topLeft.isValid || !topRight.isValid || !bottomRight.isValid || !bottomLeft.isValid || !center.isValid) {
            return
        }

        var nextIndex = _missionController.currentPlanViewVIIndex + 1
        var survey = _missionController.insertComplexMissionItem("Survey", center, nextIndex, true /* makeCurrentItem */)
        if (!survey || !survey.surveyAreaPolygon) {
            return
        }

        var polygon = survey.surveyAreaPolygon
        polygon.beginReset()
        polygon.clear()
        polygon.appendVertices([topLeft, topRight, bottomRight, bottomLeft])
        polygon.endReset()
        polygon.verifyClockwiseWinding()
        _missionController.setCurrentPlanViewSeqNum(survey.sequenceNumber, true)
    }
    ...
    Item {
        ...
        FlightMap {
            ...
            // While the Area Route tool is active, consume one drag gesture
            // and turn its GPS bounds into a native Survey coverage route.
            MouseArea {
                id: areaRouteSelectionArea

                anchors.fill: parent
                z: QGroundControl.zOrderWidgets - 1
                enabled: _addAreaRouteOnDrag && _editingLayer == _layerMission
                ...
                property point _currentPoint: Qt.point(0, 0)
                ...
                onPositionChanged: mouse => {
                    if (_dragging) {
                        _currentPoint = Qt.point(mouse.x, mouse.y)
                    }
                }
                ...
                onReleased: mouse => {
                    if (_dragging) {
                        _currentPoint = Qt.point(mouse.x, mouse.y)
                        _dragging = false
                        createAreaRoute(_startPoint, _currentPoint)
                        _addAreaRouteOnDrag = false
                    }
                    mouse.accepted = true
                }
                ...
                Rectangle {
                    x: Math.min(areaRouteSelectionArea._startPoint.x, areaRouteSelectionArea._currentPoint.x)
                    y: Math.min(areaRouteSelectionArea._startPoint.y, areaRouteSelectionArea._currentPoint.y)
                    width: Math.abs(areaRouteSelectionArea._currentPoint.x - areaRouteSelectionArea._startPoint.x)
                    height: Math.abs(areaRouteSelectionArea._currentPoint.y - areaRouteSelectionArea._startPoint.y)
                    visible: areaRouteSelectionArea._dragging
                    color: Qt.rgba(QGroundControl.globalPalette.buttonHighlight.r,
                                   QGroundControl.globalPalette.buttonHighlight.g,
                                   QGroundControl.globalPalette.buttonHighlight.b,
                                   0.18)
                    border.width: 2
                    border.color: QGroundControl.globalPalette.buttonHighlight
                }
                ...
                Rectangle {
                    ...
                    visible: !areaRouteSelectionArea._dragging
                    width: areaRouteSelectionHint.implicitWidth + (ScreenTools.defaultFontPixelWidth * 2)
                    height: areaRouteSelectionHint.implicitHeight + ScreenTools.defaultFontPixelWidth
                    color: QGroundControl.globalPalette.buttonHighlight
                    radius: ScreenTools.defaultBorderRadius

                    QGCLabel {
                        id: areaRouteSelectionHint
        ...
        ToolStrip {
            ...
            Binding {
                target: areaRouteButton
                property: "checked"
                value: _addAreaRouteOnDrag
            }
            ...
            ToolStripActionList {
                ...
                model: [
                    ...
                    ToolStripAction {
                        id: areaRouteButton
                        objectName: "planToolStrip_areaRouteButton"
                        text: qsTr("区域航线")
                        iconSource: "/qmlimages/MapDrawShape.svg"
                        enabled: _missionController.flyThroughCommandsAllowed
                        visible: toolStrip._isMissionLayer
                        checkable: true
                        onTriggered: {
                            _addAreaRouteOnDrag = !_addAreaRouteOnDrag
                            if (_addAreaRouteOnDrag) {
                                _addWaypointOnClick = false
                                _addROIOnClick = false
                            }
                        }

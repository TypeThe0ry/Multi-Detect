...
doInstall:
    ...
    SetOutPath $INSTDIR
    ; Install payload
    ; CMake's aggregate staging tree also contains dependency headers, static
    ; import libraries and non-redistributable Debug CRT files. They are build
    ; artifacts, not runtime payload, and must never enter the field installer.
    File /r \
        /x include /x lib \
        /x ${EXENAME}.pdb /x ${EXENAME}.lib /x ${EXENAME}.exp \
        /x msvcp140d.dll /x msvcp140_1d.dll /x msvcp140_2d.dll \
        /x msvcp140d_atomic_wait.dll /x msvcp140d_codecvt_ids.dll \
        /x vcruntime140d.dll /x vcruntime140_1d.dll /x concrt140d.dll \
        ${DESTDIR}\*.*

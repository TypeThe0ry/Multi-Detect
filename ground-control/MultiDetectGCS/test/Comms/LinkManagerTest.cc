...
void LinkManagerTest::_testSuspensionBlocksNewConnection()
{
    MockConfiguration *const mockConfig = new MockConfiguration(QStringLiteral("SuspendedMock"));
    SharedLinkConfigurationPtr config = linkManager()->addConfiguration(mockConfig);

    linkManager()->setConnectionsSuspended(QStringLiteral("test suspension"));
    const bool connected = linkManager()->createConnectedLink(config);

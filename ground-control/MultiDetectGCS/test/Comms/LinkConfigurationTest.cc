...
void LinkConfigurationTest::_testTcpWorkerIgnoresApplicationProxy()
{
    QTcpServer server;
    QVERIFY(server.listen(QHostAddress::LocalHost, 0));

    TCPConfiguration config(QStringLiteral("TCPNoProxy"));
    config.setHost(QHostAddress(QHostAddress::LocalHost).toString());
    config.setPort(server.serverPort());

    // Reproduce a Windows system-proxy configuration which is not valid for
    // a raw vehicle TCP socket. The worker must still connect directly.
    const QNetworkProxy previousApplicationProxy = QNetworkProxy::applicationProxy();
    const auto restoreProxy =
        qScopeGuard([previousApplicationProxy] { QNetworkProxy::setApplicationProxy(previousApplicationProxy); });
    QNetworkProxy::setApplicationProxy(QNetworkProxy(QNetworkProxy::HttpCachingProxy, QStringLiteral("127.0.0.1"), 1));

    TCPWorker worker(&config);
    QSignalSpy connectedSpy(&worker, &TCPWorker::connected);
    QSignalSpy errorSpy(&worker, &TCPWorker::errorOccurred);

    worker.setupSocket();

    // Localhost connections are implicitly direct in Qt, so connection success
    // alone would not prove that arbitrary vehicle addresses bypass the proxy.
    // Inspect the socket policy itself before making the test connection.
    QTcpSocket* const socket = worker.findChild<QTcpSocket*>();
    QVERIFY(socket);
    QCOMPARE(socket->proxy().type(), QNetworkProxy::NoProxy);

    worker.connectToHost();

    QVERIFY_SIGNAL_COUNT_WAIT(connectedSpy, 1, TestTimeout::shortMs());
    QCOMPARE(errorSpy.count(), 0);
    QVERIFY_TRUE_WAIT(server.hasPendingConnections(), TestTimeout::shortMs());

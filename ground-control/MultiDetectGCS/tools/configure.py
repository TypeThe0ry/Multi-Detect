...
def configure(config: CMakeConfig) -> int:
    ...
    # Never enable CMake's directory-wide unity mode: it also merges third-party
    # C targets such as zlib, whose internal translation-unit symbols conflict.
    args.append("-DCMAKE_UNITY_BUILD=OFF")
    if config.unity_build:
        args.append("-DQGC_UNITY_BUILD=ON")
        args.append(f"-DQGC_UNITY_BUILD_BATCH_SIZE={config.unity_batch_size}")
    else:
        args.append("-DQGC_UNITY_BUILD=OFF")

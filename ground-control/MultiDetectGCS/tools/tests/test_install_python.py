...
import pytest
from setup.install_python import (
    ensure_venv_has_pip,
    get_packages_for_groups,
    sync_groups_with_uv,
)
...
def test_missing_pip_is_seeded_into_an_existing_uv_environment(tmp_path: Path) -> None:
    venv = tmp_path / ".venv"
    with (
        patch("setup.install_python.venv_has_pip", side_effect=[False, True]),
        patch("setup.install_python.has_uv", return_value=True),
        patch("setup.install_python.subprocess.run") as mock_run,
    ):
        ensure_venv_has_pip(venv)
    ...
    mock_run.assert_called_once_with(
        [
            ...
            "install",
            "--python",
            str(venv / ("Scripts/python.exe" if sys.platform == "win32" else "bin/python")),
            "pip",
            "setuptools",
            "wheel",
        ],
...
def test_existing_pip_does_not_reseed_the_environment(tmp_path: Path) -> None:
    with (
        patch("setup.install_python.venv_has_pip", return_value=True),
        patch("setup.install_python.subprocess.run") as mock_run,
    ):
        ensure_venv_has_pip(tmp_path / ".venv")

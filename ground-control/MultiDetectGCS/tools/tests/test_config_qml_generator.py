...
class TestRealPageDefinitions:
    ...
    def test_px4_safety_allows_vehicle_types_without_collision_prevention(self):
        safety_path = (
            REPO_ROOT
            / "src"
            / "AutoPilotPlugins"
        ...
        page = load_page_def(safety_path)

        assert page.params["_cpDist"].name == "CP_DIST"
        assert page.params["_cpDist"].required is False

        qml = generate_config_page_qml(page)
        assert "enableCheckBoxChecked: _cpDist !== null && _cpDist.rawValue > 0" in qml
        assert (
            "onEnableCheckboxClicked: { if (_cpDist !== null) _cpDist.rawValue = "
            "enableCheckBoxChecked ? 5 : -1 }"
        ) in qml

...
def test_install_just_debian_upgrades_when_existing_too_old() -> None:
    ...
    with (
        ...
        patch("setup.install_dependencies._debian.platform.machine", return_value="x86_64"),
...
def test_install_windows_nsis_dry_run(monkeypatch, tmp_path: Path) -> None:
    # Keep the test independent of whether the developer workstation already
    # has NSIS installed. The installed-path branch is covered separately.

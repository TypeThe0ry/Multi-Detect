...
def venv_has_pip(venv_path: Path) -> bool:
    """Return whether the virtualenv interpreter can import pip."""
...
def ensure_venv_has_pip(venv_path: Path) -> None:
    """Restore pip in an existing uv environment when a build dependency needs it."""
    if venv_has_pip(venv_path):
        return

    print(f"Seeding pip into virtual environment: {venv_path}")
    if has_uv():
        python = get_python_executable(venv_path)
        subprocess.run(
            [
    ...
    if not venv_has_pip(venv_path):
        raise RuntimeError(f"pip is still unavailable in {venv_path}")

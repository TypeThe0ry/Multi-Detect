...
def find_repo_root(start_path: Path | None = None) -> Path:
    """
    Find the source root by looking for a root marker.

    A standalone checkout uses ``.git``. The unified Multi-Detect repository
    embeds this source tree below its own Git root, so it provides
    ``.multidetect-gcs-root`` to stop tools from resolving the parent repository
    as the QGroundControl source root.
    ...
    for parent in [current, *current.parents]:
        if (
            (parent / ".multidetect-gcs-root").exists()
